...
def toy_sequence_data():
    ...
    while True:
        ...
        if random.random() < .5:
            ...
            # Masking value: -1.
            seq = np.pad(seq, mode="constant", pad_width=(0, seq_max_len-seq_len),
            ...
            # Pad sequence until the maximum length for dimension consistency.
            # Masking value: -1
            seq = np.pad(seq, pad_width=(0, seq_max_len-seq_len), mode='constant',
                         constant_values=masking_val)
            label = 1
        yield np.array(seq, dtype=np.float32), \
...
def run_optimization(x, y):
    with tf.GradientTape() as g:
        # Forward pass.
        pred = lsmt_net(x, is_training=True)
        # Compute loss.
        loss = cross_entropy_loss(pred, y)

    # Variables to update, i.e. trainable variables.
    trainable_variables = lsmt_net.trainable_variables
...
for step, (batch_x, batch_y) in enumerate(train_data.take(training_steps), start=1):
    ...
    if step % display_step == 0:
        pred = lsmt_net(batch_x, is_training=True)
