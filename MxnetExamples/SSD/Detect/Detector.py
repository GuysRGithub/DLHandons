...
from AI.MxnetExamples.SSD.Dataset.TestDB import TestDB
from AI.MxnetExamples.SSD.Dataset.TestDB import TestDB
from AI.MxnetExamples.SSD.Dataset.Iterator import DetIter
import logging
import cv2
...
class Detector(object):
    ...
    def __init__(self, symbol, model_prefix, epoch, data_shape,
                 mean_pixels, batch_size=1, ctx=None):
        self.ctx = ctx
        if self.ctx is None:
            self.ctx = mx.cpu()
        load_symbol, args, aux = mx.model.load_checkpoint(model_prefix, epoch)
        if symbol is None:
            symbol = load_symbol
        self.mod = mx.mod.Module(symbol, context=self.ctx)
        if not isinstance(data_shape, type):
            data_shape = (data_shape, data_shape)
        self.data_shape = data_shape
        self.mod.bind(data_shapes=[('data', (batch_size, 3, data_shape[0], data_shape[1]))])
        self.mod.set_params(args, aux)
    ...
    def detect_iter(self, det_iter, show_timer=False):
        ...
        :param det_iter: DetIter
            Iterator for all testing images
        :param show_timer: Boolean
        ...
        if show_timer:
            logging.info("Detection time for {} images: {:.4f} sec"
                         .format(num_images, time_elapsed))
    ...
    def im_detect(self, im_list, root_dir=None, extension=None,
        ...
        """
        test_db = TestDB(im_list, root_dir, extension)
        test_iter = DetIter(test_db, 1, self.data_shape, mean_pixels=self.mean_pixels,
                            is_train=False)
        return self.detect_iter(det_iter=test_iter, show_timer=show_timer)

    @staticmethod
    def visualize_detection(img, detections, classes=None, thresh=0.6):
        if classes is None:
            classes = []
    ...
    def filter_positive_detections(detections):
        """
        First column (id class) is -1 for negative detections
        :param detections:
        :return:
    ...
    def detect_and_visualize(self, im_list, root_dir=None, extension=None,
                             classes=None, thresh=0.6, show_timer=False):
        """
        ...
        if classes is None:
            classes = []
        detections = self.im_detect(im_list, root_dir, extension=extension, show_timer=show_timer)
        if not isinstance(im_list, list):
            im_list = [im_list]
        assert len(detections) == len(im_list)
        for k, det in enumerate(detections):
            img = cv2.imread(im_list[k])
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            self.visualize_detection(img, detections, classes, thresh)
