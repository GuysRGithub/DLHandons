...
class RandCropper(RandSampler):
    ...
    def __init__(self, min_scale=1.0, max_scale=1.0,
        ...
        assert 0 < max_scale <= 1, "max_scale must in (0, 1]"
        assert 0 < min_scale <= 1, "min_scale must in (0, 1]"
        self.min_scale = min_scale
        self.max_scale = max_scale
        assert 0 < min_aspect_ratio <= 1, "min_ratio must in (0, 1]"
        assert 1 <= max_aspect_ratio, "max_ratio must >= 1"
        self.min_aspect_ratio = min_aspect_ratio
        self.max_aspect_ratio = max_aspect_ratio
        assert 0 <= min_overlap <= 1, "min_overlap must in [0, 1]"
        ...
        for trial in range(self.max_trials):
            ...
            if ious is not None:
                ...
                for i in range(valid_mask.size):
                    if ious[i] > 0:
                        x_min = max(0., (gt[i, 1] - l) / new_width)
                        y_min = max(0., (gt[i, 2] - t) / new_height)
                        x_max = min(1., (gt[i, 3] - t) / new_width)
                        y_max = min(1., (gt[i, 4] - t) / new_height)
                        new_gt_boxes.append([gt[i, 0], x_min, y_min, x_max, y_max])
                    if not new_gt_boxes:
                        continue
    ...
    def _check_satisfy(self, rand_box, gt_boxes):
        ...
        union_area += (gt_boxes[:, 3] - gt_boxes[:, 1]) \
        ...
        if max_iou < self.min_overlap:
            return None
        if self.config['gt_constraints'] == 'center':
            for i in range(ious.shape[0]):
                if ious[i] > 0:
                    ...
                    if gt_x < l or gt_x > r or gt_y < t or gt_y > b:
                        return None
        elif self.config['gt_constraints'] == 'corner':
            for i in range(ious.shape[0]):
                if ious[i] > 0:
...
class RandPadder(RandSampler):
    ...
    def __init__(self, min_scale=1., max_scale=1., min_aspect_ratio=1.,
        ...
        assert min_scale <= max_scale, "minS_cale must <= max_scale"
        assert min_scale >= 1, "min_scale must in (0, 1]"
        self.min_scale = min_scale
        self.max_scale = max_scale
        assert 0 < min_aspect_ratio <= 1, "min_ratio must in (0, 1]"
        assert 1 <= max_aspect_ratio, "max_aspect_ratio must >= 1"
        self.min_aspect_ratio = min_aspect_ratio
        self.max_aspect_ratio = max_aspect_ratio
        assert 0 <= min_gt_scale <= 1, "min_gt_scale must in [0, 1]"
    ...
    def sample(self, label):
        ...
        for trial in range(self.max_trials):
            if count >= self.max_sample:
                return samples
            scale = np.random.uniform(self.min_scale, self.max_scale)
            min_ratio = max(self.max_aspect_ratio, scale * scale)
            ...
            right = left + width
            bottom = top + height
            rand_box = (left, top, ratio, bottom)
            valid_mask = np.where(label[:, 0] > -1)[0]
            gt = label[valid_mask, :]
            new_gt_boxes = []
            for i in range(gt.shape[0]):
                x_min = (gt[i, 1] - left) / width
                y_min = (gt[i, 2] - top) / height
                x_max = (gt[i, 3] - left) / width
                y_max = (gt[i, 4] - top) / height
                new_size = min(x_max - x_min, y_max - y_min)
            ...
            new_gt_boxes = np.array(new_gt_boxes)
            label = np.lib.pad(new_gt_boxes, ((0, label.shape[0] - new_gt_boxes.shape[0]),
                                              (0, 0)), 'constant', constant_values=(-1, -1))
            samples.append((rand_box, label))
            count += 1
        return samples
