...
import os
import sys
import AI.MxnetExamples.SSD.Tools.FindMxnet
from AI.MxnetExamples.SSD.Detect.Detector import Detector
from AI.MxnetExamples.SSD.Symbol.SymbolFactory import get_symbol
from AI.MxnetExamples.SSD.Detect.CV2Iterator import CameraIterator
import logging
import cv2


def get_detector(net, prefix, epoch, data_shape, mean_pixels, ctx, num_class,
                 nms_thresh=0.5, force_nms=False, nms_topk=400):
    """
    ...
    if net is not None:
        ...
        net = get_symbol(network=net, data_shape=data_shape, num_classes=num_class,
                         nms_thresh=nms_thresh, force_nms=force_nms, nms_topk=nms_topk)
        detector = Detector(net, prefix, epoch, data_shape, mean_pixels,
                            ctx=ctx)
        return detector
...
def parse_class_names(class_names):
    ...
    if len(class_names) > 0:
        ...
        raise RuntimeError("No valid class_name provide")
    return class_names
...
def parse_frame_resize(x):
    ...
    assert 1 <= len(x) <= 2, 'frame_resize should be a float scaling factor'
    if len(x) == 1:
        x = x[0]
    return x
...
def parse_data_shape(data_shape_str):
    ...
    :param data_shape_str: str data_shape
    :return: tup or int
    """
...
def draw_detection(frame, det, class_names):
    (k_lass, score, x0, y0, x1, y1) = det
    k_lass_name = class_names[int(k_lass)]
    h = frame.shape[0]
    w = frame.shape[1]

    p0 = tuple(map(int, (x0 * w, y0 * h)))
    p1 = tuple(map(int, (x1 * w, y1 * h)))
    logging.info("Detection: %s %s", k_lass_name, score)
    cv2.rectangle(frame, p0, p1, (0, 0, 255), 2)
    tp0 = (p0[0], p0[1] - 5)
    draw_text = "{} {}".format(k_lass_name, score)
    cv2.putText(frame, draw_text, tp0, cv2.FONT_HERSHEY_COMPLEX_SMALL,
                0.5, (0, 0, 255))
...
def run_camera(args, ctx):
    ...
    cameraIter = CameraIterator(frame_resize=parse_frame_resize(args.frame_resize))
    class_names = parse_class_names(args.class_names)
    mean_pixels = (args.mean_r, args.mean_g, args.mean_b)
    data_shape = int(args.data_shape)
    batch_size = int(args.batch_size)
    detector = Detector(get_symbol(args.network, data_shape, num_classes=len(class_names)),
...
def run_images(args, ctx):
    ...
    assert len(image_list) > 0, "Np valid image specified to detect"
