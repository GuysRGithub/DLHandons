...
import numpy as np
import cv2
from AI.MxnetExamples.SSD.Tools.RandSampler import RandSampler
...
class DetRecordIter(mx.io.DataIter):
    ...
    def __init__(self, path_imgrec, batch_size, data_shape, path_imglist="",
                 ...
                 resize_mode='force', mean_pixels=None, **kwargs):
    ...
    def _data_augmentation(self, data, label):
        ...
        if self.is_train and self._rand_samplers:
            ...
            if num_rand_crops > 0:
                ...
                x_min = int(crop[0] * width)
                y_min = int(crop[1] * height)
                x_max = int(crop[2] * width)
                y_max = int(crop[3] * height)
                if x_min >= 0 and y_min >= 0 and x_max <= width and y_max <= height:
                    data = mx.img.fixed_crop(data, x_min, y_min,
            ...
            interp_methods = interp_methods[int(np.random.uniform(0, 1) * len(interp_methods))]
            data = mx.img.imresize(data, self._data_shape[1], self._data_shape[0], interp_methods)
