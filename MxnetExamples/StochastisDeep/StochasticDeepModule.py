...
class StochasticDeepModule(mx.module.BaseModule):
    ...
    def __init__(self, symbol_compute, symbol_skip=None,
                 ...
                 death_rate=0):
        super(StochasticDeepModule, self).__init__(logger=logger)
    ...
    def update_metric(self, eval_metric, labels, *args):
        self._module_compute.update_metric(eval_metric, labels)
        if self._module_skip:
            self._module_skip.update_metric(eval_metric, labels)

    def get_outputs(self, merger_multi_context=True):
        assert merger_multi_context, "Force merging for now"
        return self._outputs

    def get_input_grads(self, merge_multi_context=True):
        assert merge_multi_context, "Force merging ofr now"
